...
{
public:
    BRION_API Serializable();
    BRION_API virtual ~Serializable();
    ...
    BRION_API virtual uint128_t getTypeIdentifier() const;
     ...
     */
    BRION_API bool fromBinary(const Data& data);
    BRION_API bool fromBinary(const void* data, const size_t size);
     ...
     */
    BRION_API Data toBinary() const;

    /**
     * Update this serializable from its JSON representation.
     * @return true on success, false on error.
     */
    BRION_API bool fromJSON(const std::string& json);

    /** @return the JSON representation of this serializable. */
    BRION_API std::string toJSON() const;
    //@}
     ...
     */
    BRION_API void registerDeserializedCallback(const DeserializedCallback&);
     ...
     * callback is not 'nullptr' (or 0)
     */
    BRION_API void registerSerializeCallback(const SerializeCallback&);
    //@}

protected:
    BRION_API Serializable(const Serializable&);
    BRION_API Serializable& operator=(const Serializable&);
    BRION_API Serializable(Serializable&&);
    BRION_API Serializable& operator=(Serializable&&);
