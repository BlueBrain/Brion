...
public:
    ...
    uint128_t& operator=(const brion::uint128_t& rhs)
    {
        _high = rhs._high;
        _low = rhs._low;
        return *this;
    ...
    BRION_API uint128_t& operator=(const std::string& from);

    /**
     * @return true if the values are equal, false if not.
     **/
    bool operator==(const brion::uint128_t& rhs) const
    {
    ...
    std::string getString() const
    {
        ...
        unsigned long long l = low();
        snprintf(buffer, 34, "%llx:%llx", h, l);
...
inline uint128_t operator+(const brion::uint128_t& a, const uint64_t& b)
{
    uint128_t result = a;
    result.low() += b;
    if (result.low() < a.low())
...
inline uint128_t operator+(const brion::uint128_t& a,
                           const brion::uint128_t& b)
{
    uint128_t result = a;
    result += b;
    return result;
 ...
 */
BRION_API uint128_t make_uint128(const char* string);
 ...
 * identifier.
 */
BRION_API uint128_t make_UUID();
}

namespace std
{
template <>
struct hash<brion::uint128_t>
{
    typedef size_t result_type;

    result_type operator()(const brion::uint128_t& in) const
    {
...
inline string to_string(const brion::uint128_t& value)
{
